  ...
  useEffect(() => {
    (async () => {
      ...
      const upcoming = matches
        .map((m) => ({ ...m, dateObj: m.date ? new Date(m.date) : null }))
        .filter((m) => m.dateObj && m.dateObj >= now)
        .sort((a, b) => a.dateObj - b.dateObj)[0];
      setNextMatch(upcoming || matches[0] || null);

      // count total recorded results across matches (kept for debugging)
      // const totalResults = matches.reduce(
      //   (acc, m) => acc + ((m.results && m.results.length) || 0),
      //   0,
      // );
          ...
          >
            ...
            <div className="d-flex flex-column gap-2">
              ...
              <button
                className="btn btn-outline-light"
                onClick={() =>
                  onNavigate({ name: "matches", openCreate: true })
                }
                style={{
                  padding: "0.75rem 1.2rem",
                  borderRadius: "8px",
